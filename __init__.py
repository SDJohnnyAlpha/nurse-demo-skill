...
class NurseDemo(MycroftSkill):
    def __init__(self):
        super(NurseDemo, self).__init__(name="NurseDemo")

        def initialize(self):
            where_am_i_intent = IntentBuilder("WhereAmIintent").require("where.am.i").build()self.register_intent(where_am_i_intent, self.handle_where_am_i_intent)
